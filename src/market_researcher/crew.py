...
from market_researcher.tools.custom_tool import BSEWebTool, SerperSearchTool, MoneycontrolWebTool, HinduNewsTool  # Example: your web tool for BSE
...
class BSEMarketResearcher():
    ...
    def __init__(self):
        ...
        tasks_path = Path(__file__).parent / "config" / "bse_tasks.yaml"
        with open(tasks_path, "r", encoding="utf-8") as f:
            self.tasks_config = yaml.safe_load(f)
        # Load only BSE agents config
        agents_path = Path(__file__).parent / "config" / "bse_agents.yaml"
